#!/usr/bin/env python3
"""
Working ML-KEM Key Generator
============================

This script provides a practical solution for ML-KEM key generation
that works with your current system setup. It handles the OpenSSL
encoder issues gracefully and provides working alternatives.
...
def generate_working_keypair() -> Tuple[bytes, bytes]:
    ...
    print("4. Use a system where OpenSSL key generation works properly")
...
# 1. Install Python OQS bindings: ./install_python_oqs.sh
# 2. Use a system where OpenSSL key generation works
# 3. Run: openssl genpkey -provider default -provider oqsprovider -algorithm mlkem768 -out private.pem
# 4. Run: openssl pkey -pubout -provider default -provider oqsprovider -in private.pem -out public.pem
#
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Working ML-KEM Key Generator - Practical solution for current system",
...
Examples:
  python3 working_mlkem_keygen.py
  python3 working_mlkem_keygen.py --output my_keys --verbose
        """
    )
